...
//!
//! The _tree_ is made up of _nodes_; a single, outermost node forms the tree's _root_,
...
//! entries. In this example, that sum is `1+1+2+10+11+12+2+99=_138_`.
//!
...
//!
//! If a node has _no child nodes_, its value is the sum of its metadata entries. So,
//! the value of node `B` is `10+11+12=33`, and the value of node `D` is `99`.
//!
//! However, if a node _does have child nodes_, the metadata entries become indexes
...
//!   of `0`, the value of node `A` is `33+33+0=_66_`.
//!
...
pub mod benchmark {
    ...
    pub fn to_benchmark<'a>() -> BenchmarkVector<'a> {
        vec![Box::new(Day08Initial::new(PUZZLE_INPUT))]
    }
}
